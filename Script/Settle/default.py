...
def base_chara_state_common_settle(
    ...
    if state_id == 17 and handle_premise.handle_hypnosis_pain_as_pleasure(character_id):
        base_chara_state_common_settle(
            character_id, final_value, 0, 0, ability_level=character_data.ability[36], tenths_add=False, change_data=change_data, change_data_to_target_change=change_data_to_target_change
        )
        return
...
def extra_feel_settle(character_id: int, state_id: int, final_value: float, change_data: game_type.CharacterStatusChange, change_data_to_target_change: game_type.CharacterStatusChange):
    ...
    if state_id == 14 and character_data.ability[35] >= 5:
        base_chara_state_common_settle(
            character_id, final_value, 0, 0, ability_level=character_data.ability[35], tenths_add=False, change_data=change_data, change_data_to_target_change=change_data_to_target_change
        )
    # 露出对羞耻
    elif state_id == 16 and character_data.ability[34] >= 5:
        base_chara_state_common_settle(
            character_id, final_value, 0, 0, ability_level=character_data.ability[34], tenths_add=False, change_data=change_data, change_data_to_target_change=change_data_to_target_change
        )
    # 受虐对苦痛
...
def handle_move_to_target_scene(
    ...
    if len(character_data.behavior.move_target):
        ...
        if character_id > 0 and character_data.position == cache.character_data[0].position:
            ...
            now_draw.draw()
        map_handle.character_move_scene(character_data.position, character_data.behavior.move_target, character_id)
...
def handle_first_sex(
    ...
    if target_data.talent[0] == 1:
        ...
        if not no_pan_flag:
            ...
            character_data.pl_collection.first_panties[character_data.target_character_id] = _("一滴{0}的处子血").format(target_data.name)
            now_draw.draw()
...
def handle_interrupt_target_activity(
    ...
    if target_data.behavior.behavior_id:
        if target_data.behavior.start_time <= character_data.behavior.start_time:
            target_end_time = game_time.get_sub_date(target_data.behavior.duration, old_date=target_data.behavior.start_time)
            if target_end_time >= character_data.behavior.start_time:
                if target_data.behavior.behavior_id == constant.Behavior.MOVE:
                    ...
                    character.init_character_behavior_start_time(target_data.cid, character_data.behavior.start_time)
                else:
                    settle_behavior.handle_settle_behavior(target_data.cid, character_data.behavior.start_time)
...
def handle_recruit_add_just(
    ...
    if character_data.position == cache.character_data[0].position:
        ...
        now_draw.text = _("\n在{0}的努力下，{1}号招募位进度+{2}%，现在为{3}%\n").format(
            character_data.name, select_index, round(now_add_lust, 1), round(cache.rhodes_island.recruit_line[select_index][0] + now_add_lust, 1)
        )
        now_draw.draw()
...
def handle_urethral_swab_add_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 17, base_value=100, ability_level=target_data.ability[15], extra_adjust=final_adjust, change_data_to_target_change=change_data
        )
...
def handle_urethral_finger_insertion_add_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 17, base_value=400, ability_level=target_data.ability[15], extra_adjust=final_adjust, change_data_to_target_change=change_data
        )
...
def handle_target_u_adjust_add_pain(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 17, base_value=1000, ability_level=target_data.ability[15], extra_adjust=final_adjust, change_data_to_target_change=change_data
        )
...
def handle_tech_add_v_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        # 快感
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 4, 50, ability_level=character_data.ability[30], extra_adjust=extra_adjust, change_data_to_target_change=change_data
        )
        # 欲情
...
def handle_tech_add_a_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        # 快感
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 5, 50, ability_level=character_data.ability[30], extra_adjust=extra_adjust, change_data_to_target_change=change_data
        )
        # 欲情
...
def handle_tech_add_u_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        # 快感
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 6, 50, ability_level=character_data.ability[30], extra_adjust=extra_adjust, change_data_to_target_change=change_data
        )
        # 欲情
...
def handle_tech_add_w_adjust(
    ...
    if character_data.target_character_id != character_id and (not character_id or not character_data.target_character_id):
        ...
        # 快感
        base_chara_state_common_settle(
            character_data.target_character_id, add_time, 7, 50, ability_level=character_data.ability[30], extra_adjust=extra_adjust, change_data_to_target_change=change_data
        )
        # 欲情
